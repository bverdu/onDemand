...
Simple Resource Directory (draft-ietf-core-resource-directory-01)
                implementation for txThings
    ...
    def render_POST(self, request):
        ...
        if request.opt.content_format is not coap.media_types_rev['application/link-format']:
            ...
            response = coap.Message(
                code=coap.UNSUPPORTED_CONTENT_FORMAT, payload='')
        ...
        try:
            ...
            response = coap.Message(
                code=coap.BAD_REQUEST, payload="Bad or ambiguous query options!")
            return defer.succeed(response)

        if 'ep' not in params:
            log.msg("No 'ep' query option in request.")
            response = coap.Message(
                code=coap.BAD_REQUEST, payload="No 'ep' query option in request.")
            return defer.succeed(response)
        endpoint = params['ep']
        ...
        context = params.get(
            'con', 'coap://' + request.remote[0] + ":" + str(request.remote[1]))

        entry = DirectoryEntryResource(
            self, link_format, endpoint, domain, endpoint_type, lifetime, context)
...
class DirectoryEntryResource (resource.CoAPResource):
    ...
    def render_PUT(self, request):
        ...
        self.context = params.get(
            'con', 'coap://' + request.remote[0] + ":" + str(request.remote[1]))

        response = coap.Message(code=coap.CHANGED, payload='')
        log.msg("RD entry updated: endpoint=%s, lifetime=%d" %
...
class EndpointLookupResource (resource.CoAPResource):
    ...
    def render_GET(self, request):
        ...
        response = coap.Message(
            code=coap.CONTENT, payload=''.join(link_format))
...
reactor.listenUDP(coap.COAP_PORT, protocol)  # @UndefinedVariable
