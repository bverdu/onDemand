...
class Response():
    ...
    def is_null(self):
        return (self.message is None) or (self.value is None)
...
def numBitsSet(n):
    ...
    total = 0
    ref = 1
    for b in xrange(8):  # @UnusedVariable
...
def constrainHex(_hex, b, test=False):
    ...
    if diff > 0:
        if test:
            raise ValueError
        print("Received less data than expected, trying to parse anyways...")
        _hex += ('0' * diff)  # pad the right side with zeros
    elif diff < 0:
        if test:
            raise ValueError
        print("Received more data than expected, trying to parse anyways...")
