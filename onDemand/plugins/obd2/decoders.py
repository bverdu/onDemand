...
from utils import Status, Unit, Test, unhex, bitstring, twos_comp, unbin,\
    bitToBool
from codes import *
...
def fuel_status(_hex):
    ...
    if v <= 0:
        log.debug("Invalid fuel status response (v <= 0)", True)
        return (None, Unit.NONE)

    i = math.log(v, 2)  # only a single bit should be on

    if i % 1 != 0:
        log.debug("Invalid fuel status response (multiple bits set)", True)
        return (None, Unit.NONE)

    i = int(i)

    if i >= len(FUEL_STATUS):
        log.debug("Invalid fuel status response (no table entry)", True)
        return (None, Unit.NONE)
...
def air_status(_hex):
    ...
    if v <= 0:
        log.debug("Invalid air status response (v <= 0)", True)
        return (None, Unit.NONE)

    i = math.log(v, 2)  # only a single bit should be on

    if i % 1 != 0:
        log.debug("Invalid air status response (multiple bits set)", True)
        return (None, Unit.NONE)

    i = int(i)

    if i >= len(AIR_STATUS):
        log.debug("Invalid air status response (no table entry)", True)
        return (None, Unit.NONE)
