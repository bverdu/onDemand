...
class LegacyProtocol(Protocol):
    ...
    def create_frame(self, raw):
        ...
        if len(raw_bytes) < 6:
            self.log.debug("Dropped frame for being too short")
            return None

        if len(raw_bytes) > 11:
            self.log.debug("Dropped frame for being too long")
            return None
    ...
    def create_message(self, frames, tx_id):
        ...
        if len(frames) > 1:
            if not all([mode == f.data_bytes[0] for f in frames[1:]]):
                self.log.debug("Received frames from multiple commands")
                return None
        ...
        if mode == 0x43:
            ...
            if len(frames) == 1:
                ...
                # now that they're in order, accumulate the data from each
                # frame
                for f in frames:
                    # loose the mode/pid/seq bytes
...
class SAE_J1850_PWM(LegacyProtocol):
    ...
    def __init__(self):
        super(SAE_J1850_PWM, self).__init__(baud=41600)
        self.desc = "SAE_J1850_PWM" + "(" + self.family + ")"


class SAE_J1850_VPW(LegacyProtocol):

    def __init__(self):
        super(SAE_J1850_VPW, self).__init__(baud=10400)
        self.desc = "SAE_J1850_VPW" + "(" + self.family + ")"


class ISO_9141_2(LegacyProtocol):

    def __init__(self):
        super(ISO_9141_2, self).__init__(baud=10400)
        self.desc = "ISO_9141_2" + "(" + self.family + ")"


class ISO_14230_4_5baud(LegacyProtocol):

    def __init__(self):
        super(ISO_14230_4_5baud, self).__init__(baud=10400)
        self.desc = "ISO_14230_4_5baud" + "(" + self.family + ")"


class ISO_14230_4_fast(LegacyProtocol):

    def __init__(self):
        super(ISO_14230_4_fast, self).__init__(baud=10400)
        self.desc = "ISO_14230_4_fast" + "(" + self.family + ")"
