...
from onDemand.plugins.obd2.utils import contiguous, ascii_to_bytes
from protocol import Protocol, Frame, Message
...
class CANProtocol(Protocol):
    ...
    def __init__(self, baud, id_bits):
        super(CANProtocol, self).__init__(baud)
    ...
    def create_frame(self, raw):
        ...
        if frame.type not in [self.FRAME_TYPE_SF,
                              self.FRAME_TYPE_FF,
                              self.FRAME_TYPE_CF]:
            self.log.debug("Dropping frame carrying unknown PCI frame type")
            return None
    ...
    def create_message(self, frames, tx_id):
        ...
        if len(message.frames) == 1:
            ...
            if frame.type != self.FRAME_TYPE_SF:
                self.log.debug(
                    "Received lone frame not marked as single frame")
                return None

            # extract data, ignore PCI byte and anything after the marked
            # length
            ...
            for f in frames:
                if f.type == self.FRAME_TYPE_FF:
                    ...
                    self.log.debug(
                        "Dropping frame in multi-frame response" +
                        " not marked as FF or CF")

            # check that we captured only one first-frame
            if len(ff) > 1:
                self.log.debug("Recieved multiple frames marked FF")
                return None
            elif len(ff) == 0:
                self.log.debug("Never received frame marked FF")
                return None

            # check that there was at least one consecutive-frame
            if len(cf) == 0:
                self.log.debug("Never received frame marked CF")
                return None

            # calculate proper sequence indices from the lower 4 bits given
            for prev, curr in zip(cf, cf[1:]):
                # Frame sequence numbers only specify the low order bits,
                # so compute the full sequence number from the frame number
                # and the last sequence number seen:
                # 1) take the high order bits from the last_sn and low order
                # bits from the frame
                seq = (prev.seq_index & ~0x0F) + (curr.seq_index)
                # 2) if this is more than 7 frames away, we probably
                # just wrapped (e.g.,last=0x0F current=0x01 should mean 0x11,
                # not 0x01)
            ...
            # now that they're in order, load/accumulate the data from each CF
            # frame
            for f in cf:
...
class ISO_15765_4_11bit_500k(CANProtocol):
    ...
    def __init__(self):
        super(ISO_15765_4_11bit_500k, self).__init__(baud=500000, id_bits=11)
        self.desc = "ISO_15765_4_11bit_500k" + "(" + self.family + ")"


class ISO_15765_4_29bit_500k(CANProtocol):

    def __init__(self):
        super(ISO_15765_4_29bit_500k, self).__init__(baud=500000, id_bits=29)
        self.desc = "ISO_15765_4_29bit_500k" + "(" + self.family + ")"


class ISO_15765_4_11bit_250k(CANProtocol):

    def __init__(self):
        super(ISO_15765_4_11bit_250k, self).__init__(baud=250000, id_bits=11)
        self.desc = "ISO_15765_4_11bit_250k" + "(" + self.family + ")"


class ISO_15765_4_29bit_250k(CANProtocol):

    def __init__(self):
        super(ISO_15765_4_29bit_250k, self).__init__(baud=250000, id_bits=29)
        self.desc = "ISO_15765_4_29bit_250k" + "(" + self.family + ")"


class SAE_J1939(CANProtocol):

    def __init__(self):
        super(SAE_J1939, self).__init__(baud=250000, id_bits=29)
        self.desc = "SAE_J1939" + "(" + self.family + ")"
